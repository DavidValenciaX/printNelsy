...
function setImageSizeInCm() {
    ...
    if (selectedImages.length === 0) {
        Swal.fire({ text: 'Seleccione primero una o más imágenes.', icon: 'warning' });
        return;
    }
...
function setSingleImageSizeInCm(selectedImage) {
    ...
    if (widthInputValue) {
        ...
        if (isNaN(cmWidth) || cmWidth <= 0) {
            Swal.fire({ text: 'Introduzca una anchura válida en centímetros.', icon: 'warning' });
            widthInput.value = '';
            return;
        }
    ...
    if (heightInputValue) {
        ...
        if (isNaN(cmHeight) || cmHeight <= 0) {
            Swal.fire({ text: 'Introduzca una altura válida en centímetros.', icon: 'warning' });
            heightInput.value = '';
            return;
        }
    ...
    if (!updated) {
        Swal.fire({ text: 'Introduzca al menos una medida válida.', icon: 'warning' });
        widthInput.value = '';
        heightInput.value = '';
        return;
    }
    ...
    if (
        ...
        selectedImage.setCoords();
        Swal.fire({ text: 'El tamaño deseado excede el límite de los márgenes.', icon: 'warning' });
        canvas.renderAll();
        return;
    }
...
function confirmCrop() {
    if (!isCropping || !cropRect || !activeImage) {
        Swal.fire({ text: 'Seleccione primero una imagen y active el modo de recorte.', icon: 'warning' });
        return;
    }
...
function resetActiveImage() {
    ...
    activeObjects.forEach(activeObject => {
        if (!activeObject || !activeObject.id || !originalImages[activeObject.id]) {
            Swal.fire({ text: `No se pudo restablecer la imagen con id ${activeObject.id || 'n/a'}.`, icon: 'warning' });
            return;
        }
...
function scaleUp() {
    ...
    const intendedMultiplier = (activeObject.scaleX + SCALE_FACTOR) / activeObject.scaleX;
    ...
    const allowedMultiplier = Math.min(maxMultiplierLeft, maxMultiplierRight, maxMultiplierTop, maxMultiplierBottom);
...
cropButton.addEventListener('click', function () {
    ...
    if (activeObjects.length !== 1) {
        Swal.fire({ text: 'Seleccione solo una imagen para recortar.', icon: 'warning' });
        return;
    }

    const activeObject = activeObjects[0];
    if (activeObject.type !== 'image') {
        Swal.fire({ text: 'La selección debe ser una imagen válida.', icon: 'warning' });
        return;
    }
...
canvas.on('object:scaling', function (e) {
    ...
    if (isValidState()) {
        ...
        while ((high - low) > tolerance) {
            ...
            let testScaleX = lastValidScaleX + mid * (proposedScaleX - lastValidScaleX);
            let testScaleY = lastValidScaleY + mid * (proposedScaleY - lastValidScaleY);
